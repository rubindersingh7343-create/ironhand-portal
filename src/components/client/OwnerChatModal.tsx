...
const seenType: Record<ChatType, "chat-manager" | "chat-surveillance" | "chat-owner"> = {
  manager: "chat-manager",
  surveillance: "chat-surveillance",
  owner: "chat-owner",
};
...
export default function OwnerChatModal({ type, storeId, storeName, onClose }: Props) {
  ...
  const markSeen = useCallback(async (nextMessages: StoreChatMessage[]) => {
    const unseen = nextMessages.filter((msg) => msg.senderRole !== "owner");
    if (!unseen.length) return;
    await fetch("/api/owner/seen", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        items: unseen.map((msg) => ({
          storeId: msg.storeId,
          itemType: seenType[type],
          itemId: msg.id,
        })),
      }),
    });
  ...
  return (
    <IHModal isOpen onClose={onClose} allowOutsideClose>
      <div className="flex h-[80vh] max-h-[640px] w-[min(560px,92vw)] flex-col overflow-hidden">
